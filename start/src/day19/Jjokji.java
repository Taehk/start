...
public class Jjokji {
	Properties nameToIp, ipToName;	// 이름을 알려주면 주소를 찾아내는 Map
	...
	DatagramSocket sSocket, rSocket;
				// sSocket = 쪽지를 보내기 위한 소켓
				// rSocket = 쪽지를 받기 위한 소켓	
		
		// 반드시 2개를 만들어야 하는 것은 아니고
		// 소켓 하나만 가지고 있어도 보내기와 받기를 동시에 할 수 있으나
		// 하나를 공유하는 경우에는 보내는 도중에 받지를 못할 수 있다.
			// 그렇기 때문에 안전한 통신을 위해 보내/받는 소켓을 구분해서 처리하는 것이 좋다ㄴ
	public Jjokji() {
		setMap();
		...
		/*
		 		...
		 		네트워크 구성을 할 차례이다.
		 		여기서는 UDP 통신을 해야하므로
		 		DatagramSocket을 이용해서 네트워크를 구성해야한다.
		 */
		try {
			...
			ReceiveThread t = new ReceiveThread(this);	// New Born상태
			//Runnalbe 상태로 전위
			t.start();
			
		}catch(Exception e) {
			close();			// 해당 작업에 오류 = 작업 속행 불가능 = 열려있는 소켓 모두 닫기
			e.printStackTrace();
		}
	}
