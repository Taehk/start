...
/*
 		...
 		ex)
 			...
 			따라서 연산기호는 오는 위치 이전까지 문자열을 잘라서 첫번째에 기억시키고
 			연산기호를 두번째 위치에 기억시키고 나머지를 마지막 위치에 기억시켜서 처리한다
 ...
 */
	public static void main(String[] args) {
		...
		// 반복으로 추출해서 연산기호의 위치를 알아낸다.
		for(int i = 0; i < str.length(); i++) {
			char ch = str.charAt(i);
			
			if(ch < '0' || ch > '9') {	// 아스키코드 값의 문자가 숫자형태가 아니므로 연산기호임
				idx = i;
				break;
			}
		}
		...
		// 결과 출력
		System.out.println("입력한 계산식 "+sik[0]+sik[1]+sik[2]);
